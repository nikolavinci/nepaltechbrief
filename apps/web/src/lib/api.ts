...
function mapWPPostToArticle(post: any): Article {
  ...
  return {
    id: post.id,
    slug: post.slug,
    title_en: decodeHTMLEntities(post.title.rendered),
    title_np: decodeHTMLEntities(post.title.rendered),
...
export async function fetchArticles(page: number = 1, limit: number = 12, q?: string): Promise<PaginatedArticles> {
  try {
    ...
    return { data: posts.map(mapWPPostToArticle), current_page: page, last_page: totalPages, total };
  } catch (error) {
...
export async function fetchCategories(): Promise<Category[]> {
  try {
    ...
    return categories.map((cat: any) => ({
      id: cat.id, slug: cat.slug, name_en: cat.name, name_np: cat.name
    }));
  } catch (error) { return []; }
}
...
export async function fetchArticle(slug: string): Promise<Article | null> {
  try {
    const res = await fetch(API_BASE_URL + '/posts?_embed=1&slug=' + slug, { next: { revalidate: 60 } });
...
export async function fetchArticlesByAuthor(authorId: number, page: number = 1, limit: number = 12): Promise<PaginatedArticles> {
  try {
    ...
    const posts = await res.json();
    return { data: posts.map(mapWPPostToArticle), current_page: page, last_page: totalPages, total: totalItems };
  } catch (error) { return { data: [], current_page: page, last_page: page, total: 0 }; }
}
