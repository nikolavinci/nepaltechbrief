...
class CA_Rest_Bridge {
    ...
    public function ajax_manual_run() {
        ...
        wp_send_json_success("Manual Execution Completed: Discovered, Fetched, and Generated from Queues!");
    }
    ...
    public function api_publish_post( WP_REST_Request $request ) {
        ...
        if ( !empty($params['meta_description']) ) update_post_meta($post_id, '_ai_meta_description', sanitize_text_field($params['meta_description']));
        if ( !empty($params['focus_keywords']) ) update_post_meta($post_id, '_ai_focus_keyword', sanitize_text_field($params['focus_keywords']));
        if ( !empty($params['entities']) ) update_post_meta($post_id, '_ai_extracted_entities', wp_slash(json_encode($params['entities'])));
